class RPCController {
    ...
    private triggerNetEvent(pEvent: string, src: any, ...params: any[]): void {
    ...
    public async rpcExecute(name: string, src: any, ...params: any[]): Promise<any[]> {
        ...
        this.triggerNetEvent('rpc:request', src, this.resource, name, callID, params, false);
    ...
    private async rpcRequest(origin: string, name: string, callID: number, params: any[], boolean: boolean): Promise<void> {
        ...
        try {
            response = await selectFunc(src, ...params);
        } catch (error) {
        ...
        this.triggerNetEvent('rpc:response', src, origin, callID, response, boolean);
    }
